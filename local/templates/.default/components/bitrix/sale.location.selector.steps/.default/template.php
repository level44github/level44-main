...
<?else:?>
	...
	<div id="sls-<?=$arResult['RANDOM_TAG']?>" class="bx-slst<?if($arResult['ADMIN_MODE']):?> bx-admin-mode<?endif?> js-location_container">
		...
		<input type="hidden" name="<?=$arParams['INPUT_NAME']?>"
               ...
               class="bx-ui-slst-target js-form__location__value"
               data-prop="LOCATION"
        />
		...
		<script type="text/html" data-template-id="bx-ui-slst-selector-scope">
			...
			<div class="dropdown-block bx-ui-slst-input-block<?=($arParams['DISABLE_KEYBOARD_INPUT'] == 'Y' ? ' disabled-keyboard' : '')?> form-control js-form__control">
				<span class="dropdown-icon"></span>
				<input type="text" name="" value="" autocomplete="off" class="dropdown-field" />
				<div class="bx-ui-combobox-container" style="margin: 0px; padding: 0px; border: none; position: relative;width: 80%;">
					...
					<?else:?>
                        <input type="text" value="" autocomplete="off"
                               class="bx-ui-combobox-fake js-form__location is-required"
                               data-prop="LOCATION"
                               placeholder="<?= Loc::getMessage('SALE_SLS_SELECTOR_PROMPT') ?>"
                        />
					<?endif?>
				</div>
